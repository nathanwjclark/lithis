...
| **P3-connect** | in review (PR #7) | `server.connections.registry.*`, feed-expectation TickSource, `server.custody.broker.getBrokered` (secret store; `mountSession` stays stubbed), orchestrator sync loop | P1 | — | scheduled sync persists cursor + emits events; missed feed flags |
